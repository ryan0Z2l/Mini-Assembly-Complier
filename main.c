...
// Generate ASM code.
void codegen(AST *root,int *static_pointer);
// Free the whole AST.
...
void semantic_check(AST *now) {
	...
	if (now->kind == PREINC || now->kind == PREDEC || now->kind == POSTINC || now->kind == POSTDEC) {
        AST *operand = now->mid;
        while (operand->kind == LPAR) operand = operand->mid;
        if (operand->kind != IDENTIFIER)
            err("Operand of INC/DEC must be an identifier or an identifier with parentheses.");
    }
...
void codegen(AST *root, int *stack_pointer) {
    ...
    switch (root->kind) {
        case ASSIGN:
            codegen(root->rhs, stack_pointer);
            codegen(root->lhs, stack_pointer);
            printf("store [%d] r%d\n", var_mem, (*stack_pointer) - 1);
            (*stack_pointer)--;
            break;
        case ADD:
            codegen(root->lhs, stack_pointer);
        ...
        case MUL:
            codegen(root->lhs, stack_pointer);
            codegen(root->rhs, stack_pointer);
            printf("mul r%d r%d r%d\n", (*stack_pointer) - 1, (*stack_pointer), (*stack_pointer) - 1);
            (*stack_pointer)--;
            break;
        case DIV:
            codegen(root->lhs, stack_pointer);
        ...
        case REM:
            ...
            break;
        case PREINC:
            codegen(root->mid, stack_pointer);
            printf("add r%d r%d 1\n", (*stack_pointer), (*stack_pointer));
            printf("store [%d] r%d\n", var_mem, (*stack_pointer));
            break;
        case PREDEC:
            codegen(root->mid, stack_pointer);
        ...
        case POSTINC:
            ...
            printf("add r%d r%d 1\n", (*stack_pointer), (*stack_pointer));
            printf("store [%d] r%d\n", var_mem, (*stack_pointer));
            printf("sub r%d r%d 1\n", (*stack_pointer), (*stack_pointer));  // return y rather than y+1
            break;
        case POSTDEC:
            codegen(root->mid, stack_pointer);
            printf("sub r%d r%d 1\n", (*stack_pointer), (*stack_pointer));
